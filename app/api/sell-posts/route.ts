...
export async function POST(request: Request) {
  try {
    ...
    // First, create the sell post to get the ID for the upload path
    ...
    if (postError || !postData) {
      console.error("Error creating sell post:", postError);
      return NextResponse.json(
        { error: "Failed to create sell post", details: postError?.message },
        { status: 500 }
      );
    }
    ...
    if (images && images.length > 0) {
      const folderPath = `${user.id}/sell_post/${postData.id}`;
    ...
    console.error("Error in sell-posts API:", error);
    return NextResponse.json(
...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { error: "Failed to fetch sell posts" },
      { status: 500 }
    );
  }
}
